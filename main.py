...
def test_model(args, model, tokenizer, test_dataset, best_threshold=0.5):
    # build dataloader
    ...
    for mini_batch in progress_bar:
        ...
        if y_preds[0] == True:
            ...
            score_df = get_all_lines_score(word_att_scores)
            score_df = score_df.sort_values(by=['all_lines_score'], ascending=False)
            score_df['all_lines_score'] = score_df['all_lines_score'].div(score_df['all_lines_score'].iloc[0]).round(2)
            maxPredictionValues = 0.6 * score_df.shape[0]
            maxPredictionValues = math.ceil(maxPredictionValues)
            score_indices = []
            for i in range (maxPredictionValues):
                score_indices.append(score_df['line_index'].iloc[i])
            #print("fscore_df line_index: ", score_indices)
    return score_indices

def evaluate_model(args, model, tokenizer, test_dataset, best_threshold=0.5):
    # build dataloader
    test_sampler = SequentialSampler(test_dataset)
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.eval_batch_size, num_workers=0)
    ...
    totalCoverage = 0.0
    totalPrecision = 0.0
    individualRecall = 0.0
    individualCoverage = 0.0
    individualPrecision = 0.0
    F1Score = 0.0
    ...
    for mini_batch in progress_bar:
        ...
        if index in tp_indices:
            ...
            groundTruthValues = df["flaw_line_index"][index]
            groundTruthValues = groundTruthValues.split(',')
            if(int(groundTruthValues[-1]) > score_df['line_index'].iloc[-1]):
                NAIndices += 1
                individualRecall = 0.0
                individualCoverage = 0.0
                individualPrecision = 0.0
                ...
                for groundTruthValue in groundTruthValues:
                    for predictedValue in score_indices:
                        if predictedValue == int(groundTruthValue):
                            individualRecall += 1
                individualPrecision = individualRecall / len(score_indices)
                individualRecall /= len(groundTruthValues)
                individualCoverage = len(score_indices) / totalLines

        totalRecall += individualRecall
        totalCoverage += individualCoverage
        totalPrecision += individualPrecision
        individualRecall = 0.0
        individualCoverage = 0.0
        individualPrecision = 0.0
        index += 1
    if validIndices == 0:
        ...
        totalRecall /= validIndices
        totalCoverage /= validIndices
        totalPrecision /= validIndices
        F1Score = (2*totalRecall*totalPrecision)/ (totalRecall+totalPrecision)

    #TotalGT_Coverage /= index
    print("Total Coverage: ", totalCoverage)
    print("Total NA lines: ", NAIndices)
...
def get_all_lines_score(word_att_scores: list):
    ...
    for i in range(len(word_att_scores)):
        ...
        if ((word_att_scores[i][0] in separator) or (i == (len(word_att_scores) - 1))) and score_sum != 0:
            ...
            line_indices.append(line_idx+1)
            line = ""
            score_sum = 0
            line_idx += 1
        # else accumulate score
            ...
            score_sum += word_att_scores[i][1]
        score_df = pd.DataFrame({"all_lines_score": all_lines_score, "line_index": line_indices})
    return score_df
...
def main():
    ...
    #     model.to(args.device)
    #     test_dataset = TextDataset(tokenizer, args, file_type='test')
    #     evaluate_model(args, model, tokenizer, test_dataset, best_threshold=0.5)   
    if args.do_test:
        checkpoint_prefix = '12heads_linevul_model.bin'
        output_dir = checkpoint_prefix #os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        model.load_state_dict(torch.load(output_dir, map_location=args.device))
        model.to(args.device)
        # print("--------------------------------------------------------------")
        # print("test_data_file: ", args.test_data_file)
        test_dataset = TextDataset(tokenizer, args, file_type='test')
        results = test_model(args, model, tokenizer, test_dataset, best_threshold=0.5)
        print(results)
        #return results
